...
export default class PageManager {
  ...
  constructor(visual, book, p) {
    ...
    this.canvases = [];
    for(let i=0; i<10; ++i) {
      const c = GraphUtils.createCanvas();
  ...
  load(material, n) {
    const pi = this.rtlPageN(n)<this.book.getPages()? this.p.pageCallback(this.rtlPageN(n)): {type: 'blank'}, p = this.isCover(n)? this.p.cover: this.p.page;
    const o = {
      n,
  ...
  loadPredictedPages() {
    Promise.resolve().then(()=> {
      ...
      for(let p of this.predictedRequests) {
        if(p>=0 && p<this.book.getBookPages() && !this.pageCache.get(p)) {
