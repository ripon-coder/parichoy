				...
				<div class="col-md-8 m-auto">
					<div class="main-card mb-3 card">
						<div class="card-body">
							...
							<form class="" action="{{ route('admin.advertisement.store') }}" method="POST" enctype="multipart/form-data">
								@csrf
								<div class="position-relative form-group">
									<label for="name" class="">Conpany Name</label>
									<input name="name" id="name" placeholder="Enter Company name" type="text" class="form-control" value="{{ old('name') }}" required>
								</div>								
								
                                {{-- <div class="position-relative form-group">
									<label for="exampleEmail" class="">Description (Write Description, Image URL, Video URL etc.)</label>
                                    <textarea id="editor2" name="description" id="exampleText" class="form-control" cols="30" rows="5" required>{{ old('description') }}</textarea>
								</div> --}}
